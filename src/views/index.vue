        ...
        <ul class="channel-list">
          <li :class="activeLink == 0 ? 'active-channel' : ''" @click="toLink(0)">
            <a class="link-wrapper">
              <House style="width: 1.3em; height: 1.3em; margin-right: 8px" /><span class="channel"
                >发现</span
              >
            </a>
          </li>
          <li :class="activeLink == 1 ? 'active-channel' : ''" @click="toLink(1)">
            <Star style="width: 1.3em; height: 1.3em; margin-right: 8px" />
            <el-badge
              :value="messageStore.messageCount.trendCount"
              :offset="[12, 0]"
              v-if="messageStore.messageCount.trendCount > 0 && userInfo != null"
            >
              <span class="channel">动态</span></el-badge
            >
            <span class="channel" v-else>动态</span>
          </li>
          <li :class="activeLink == 2 ? 'active-channel' : ''" @click="toLink(2)">
            ...
            <el-badge
              :value="messageCount"
              :offset="[12, 0]"
              v-if="messageCount > 0 && userInfo != null"
            >
            ...
            <div class="container">
              ...
              <div>
                <div class="group-wrapper">
                  ...
                  <div v-if="userInfo != null">
                    <div class="menu-item hover-effect" @click="userLogout">
...
import { UncheckedMessageType, type Message, type MessageCount, type WSMessage } from "@/types/message";
...
watch(
  ...
  (val) => {
    if (!val[0]) return;
    const allCount =
      val[0].chatCount + val[0].likeOrFavoriteCount + val[0].commentCount + val[0].followCount;
    if (allCount === 0) {
...
const messageCount = computed({
  get: () => {
    return (
      (messageStore.messageCount.chatCount || 0) +
      (messageStore.messageCount.likeOrFavoriteCount || 0) +
      (messageStore.messageCount.commentCount || 0) +
      (messageStore.messageCount.followCount || 0)
    );
  },
...
const connectWs = (uid: string) => {
  ...
  ws.value.onmessage = (event: any) => {
    ...
    switch (wsMessage.type) {
      case UncheckedMessageType.CHAT:
        messageCount.chatCount += 1;
        messageStore.setMessage(wsMessage.content as Message);
        break;
      case UncheckedMessageType.LIKE_OR_FAVORITE:
        count = wsMessage.content as number;
        messageCount.likeOrFavoriteCount += count;
        break;
      case UncheckedMessageType.COMMENT:
        count = wsMessage.content as number;
        messageCount.commentCount += count;
        break;
      case UncheckedMessageType.FOLLOW:
        count = wsMessage.content as number;
        messageCount.followCount += count;
        break;
      case UncheckedMessageType.TREND:
        count = wsMessage.content as number;
        messageCount.trendCount += count;
        break;
    }
  };
};
...
const getWsMessage = async () => {
  ...
  await getChatConversationListMethod();
  await getMessageCountMethod();
};
