...
import { getRecommendNote, getNoteByDTO, addRecord } from "@/apis/search";
import { getCategoryTreeData } from "@/apis/category";
import type { NoteDTO, NoteSearch } from "@/types/note";
...
const loadMoreData = () => {
  ...
  if (isNaN(noteDTO.value.cpid) && noteDTO.value.keyword == "") {
    ...
    getNoteByDTO(currentPage.value, pageSize, noteDTO.value).then((res: any) => {
      setData(res);
    });
  }
};
